...
MyState handler_meas_ble_connected(void *payload) { return STATE_MEAS; }

MyState handler_meas_ble_disconnected(void *payload) { return STATE_MEAS; }
...
MyState handler_meas_timer1_timeout(void *payload)
{
  ...
  acc_comp_sum[(sys->cnt / 5) % 10] +=
      (uint16_t)(sensor->acc_comp *
                 1000); // 0.5sの間に取得した加速度センサの値の合計
  sys->cnt++;
  return STATE_MEAS;
}
